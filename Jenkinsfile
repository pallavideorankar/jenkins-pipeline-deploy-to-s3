...
pipeline {
    ...
    environment {
        ...
        AWS_DEFAULT_REGION = "eu-west-2" // AWS region to use
    }
    ...
    stages {
        ...
        stage('Deploy to S3') {
            steps {
                ...
                script {
                    sh "aws s3 cp public/index.html s3://jenkinstf-ec2-static-bucket-wallmart/"
                    sh "aws s3 cp public/error.html s3://jenkinstf-ec2-static-bucket-wallmart/"
                }
            }
        }
        ...
        stage('Set Bucket Policy') {
            steps {
                ...
                script {
                    sh "aws s3api put-bucket-policy --bucket jenkinstf-ec2-static-bucket-wallmart --policy file://policy.json"
                }
            }
        }
    }
